...
from Furby_p3.parse_cfg import parse_cfg as pcfg
...
def dm_smear_delay(chw, dm, cfreq, tsamp):
  dm_width = 8.3 * chw * dm * (cfreq / 1e3)**(-3) * 1e-3
  dms_width_samples = max( [1, int(dm_width / tsamp)] )
  return dms_width_samples

def dm_smear_channel(d_ch, dm, cfreq, chw, tsamp):
  '''
  d_ch: 1-D array -- data of a single channel
  dm: DM to dmsmear at (pc/cc)
  cfreq:  Center freq of the channel (in MHz)
  chw:    Channel bandwidth (in MHz)
  tsamp:  Sampling time of data (in ms)
  '''
  dms_width_samps = dm_smear_delay(chw, dm, cfreq, tsamp)
...
def disperse(frb, dm, pre_shift, dmsmear):
    ...
    start = int(nsamps/2) - int(pre_shift*C.tfactor)
    #end = start + frb.shape[1]
    
    dm_smear_max = dm_smear_delay(dm, chw, f_ch[-1], tres)
    ...
    for i in range(nch):
      ...
      if dmsmear:
        dmsmeared_channel = dm_smear_channel(frb[int(i/ffactor)], dm, cfreq = f_ch[i], chw=chw, tsamp=tres)
      else:
        dmsmeared_channel = frb[int(i/ffactor)].copy()
...
def pad_along_axis(array, target_length, axis=0):
    ...
    padded_array = N.pad(array, pad_width = npad, mode='constant', constant_values = 0)
...
def main(args):
    ...
    if isinstance(args.tau, float):
      tau0s = args.tau * N.ones(args.Num)
    elif isinstance(args.tau, list) and len(args.tau) ==1:
      tau0s = args.tau[0] * N.ones(args.Num)
...
# -----------------------------------------------------------------------------------------------------------------
    for num in range(args.Num):
      ...
      params = {
              ...
              "NSAMPS":	NSAMPS,
              "UTC_START":    "2022-01-01-00:00:00",       #Never happened :P
              "STATE":    "Intensity",
      ...
      if not args.plot:
          ...
          out.write(header_string.encode('ascii'))
          if order == "TF":
...
if __name__ == '__main__':
    ...
    a.add_argument("-params", type=str, help="Path to the parameter file")
    ...
    a.add_argument("-tau", nargs='+', type=float, help="Tau or tau range endpoints (in ms), default=0.1", default=0.1)
